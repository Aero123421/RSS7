...
このコマンドが送信されたチャンネルの最新記事を1件取得し、通常の要約翻訳を行って投稿します。
...
記事の翻訳を有効/無効にするには、`/rss_config`コマンドを使用して設定パネルを表示し、「翻訳」セクションで設定します。
